...
export const optimizeLabelsWithVision = async (
  ...
  try {
    console.log(`📡 Sending vision optimization request to ${backendUrl}/api/labels/position-from-image`);
    console.log(`   Model ID: ${modelId}, Concept: ${concept}, Parts: ${partDefinitions.length}`);

    const response = await fetch(`${backendUrl}/api/labels/position-from-image`, {
      method: 'POST',
